...
@pytest.mark.parametrize(
	("stringCaseName", "valueIsIdenticalInput"),
	[
		("identicalMagnitudeSpectrogram", True),
		("distinctMagnitudeSpectrogram", False),
	],
)
def test_analyze_l1_frequency_loss_returns_registered_bounded_similarity(
	...
	tensorSpectrogramMagnitudeAlfa, tensorSpectrogramMagnitudeBeta = tensorSpectrogramMagnitudeAuralossCase
	tensorSpectrogramMagnitudeTarget = (
		tensorSpectrogramMagnitudeAlfa
		if valueIsIdenticalInput
		else tensorSpectrogramMagnitudeBeta
	)
	valueLoss = analyzersUseTensorSpectrogram.analyzeL1FrequencyLoss(
		tensorSpectrogramMagnitudeAlfa,
		tensorSpectrogramMagnitudeTarget,
	)
	assert isinstance(valueLoss, float), (
		f"analyzeL1FrequencyLoss returned {type(valueLoss).__name__}, expected float for {stringCaseName}."
	)
	assert math.isfinite(valueLoss), (
		f"analyzeL1FrequencyLoss returned non-finite value {valueLoss} for {stringCaseName}."
	)
