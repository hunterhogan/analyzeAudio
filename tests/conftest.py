...
def tensorAudioAuralossCase() -> tuple[torch.Tensor, torch.Tensor, int]:
	...
	tensorAudioAlfa = torch.stack((torch.sin(tensorPhase) * 0.73, torch.cos(tensorPhase * 0.61) * 0.57), dim=0).unsqueeze(0)
	tensorAudioBeta = torch.stack(
		(torch.sin(tensorPhase * 0.97 + 0.29) * 0.69, torch.cos(tensorPhase * 0.59 + 0.41) * 0.53), dim=0
	).unsqueeze(0)
...
def waveformLibrosaCase() -> tuple[numpy.ndarray, int]:
	...
	arrayWaveform = (
		numpy.sin(arrayPhase) * 0.73 + numpy.cos((arrayPhase * 0.61) + 0.29) * 0.17 + numpy.sin((arrayPhase * 0.13) + 0.41) * 0.11
