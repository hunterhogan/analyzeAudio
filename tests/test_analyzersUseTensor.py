...
def test_auraloss_waveform_losses_return_registered_floats(
	...
	valueLoss = (
		analyzer(tensorAudioAlfa, tensorAudioBeta, sampleRate) if boolNeedsSampleRate else analyzer(tensorAudioAlfa, tensorAudioBeta)
	)
...
@pytest.mark.parametrize(
	...
	[({}, True, True), ({'fast': False}, True, True), ({'fast': False}, None, None), ({'fast': True}, None, True)],
)
def test_analyze_srmr_forwards_fast_setting_from_keyword_or_cpu_flag(
	...
	monkeypatch.setattr(analyzersUseTensor, 'speech_reverberation_modulation_energy_ratio', stubSpeechReverberationModulationEnergyRatio)

	tensorResult = analyzersUseTensor.analyzeSRMR(tensorAudioAlfa, sampleRate, pytorchOnCPU=pytorchOnCPU, **dictionaryKeywordArguments)
...
@pytest.mark.parametrize(('tensorSrmrValues', 'valueExpectedMean'), [(torch.tensor([2.0, 5.0, 11.0], dtype=torch.float32), 6.0)])
def test_analyze_srmr_mean_reduces_analyze_srmr_tensor_to_python_float(
	...
	assert isinstance(valueMean, float), f'analyzeSRMRMean returned {type(valueMean).__name__}, expected float.'
...
@pytest.mark.parametrize(('integerSampleTrimmedExpected',), [(33007,)])
def test_analyze_log_wmse_mean_truncates_and_reshapes_inputs_for_upstream_call(
	tensorAudioAuralossCase: tuple[torch.Tensor, torch.Tensor, int], monkeypatch: pytest.MonkeyPatch, integerSampleTrimmedExpected: int
) -> None:
	tensorAudioAlfa, tensorAudioBeta, sampleRate = tensorAudioAuralossCase
	...
	class StubLogWMSE:
		...
		def __call__(
			self, tensorAudioInput: torch.Tensor, tensorAudioEstimate: torch.Tensor, tensorAudioTarget: torch.Tensor
		) -> torch.Tensor:
...
@pytest.mark.parametrize(
	('integerSampleTrimmedExpected', 'valueLossInternal', 'dictionaryKeywordArguments'), [(32977, 4.9, {'l1_weight': 0.37})]
)
